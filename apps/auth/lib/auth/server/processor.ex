defmodule Auth.Server.Processor do
  @moduledoc """
  TODO
  """
  ...
  @supervisor_name Auth.Server.Processor.Supervisor
  @pool_name :auth_pool
  @pool_size 2
  @pool_worker Auth.Server.Worker
  @pool_max_overflow 0
  ...
  def process(message) do
    Logger.debug "#{__MODULE__} - Delegating the processing of an auth message to the worker pool!"
  ...
  defp do_process(message) do
    :poolboy.transaction(
      @pool_name,
      fn(pid) -> Auth.Server.Worker.process(pid, message) end,
      :infinity
    )
  end
end
