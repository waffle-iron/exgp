defmodule Router.Server.Processor do
  @moduledoc """
  TODO
  """
  ...
  @supervisor_name Router.Server.Processor.Supervisor
  @pool_name :router_pool
  @pool_size 2
  @pool_worker Router.Server.Worker
  @pool_max_overflow 0
  ...
  def process(message) do
    Logger.debug "#{__MODULE__} - Delegating the processing of an router message to the worker pool!"
  ...
  defp do_process(message) do
    :poolboy.transaction(
      @pool_name,
      fn(pid) -> Router.Server.Worker.process(pid, message) end,
      :infinity
    )
  end
end
